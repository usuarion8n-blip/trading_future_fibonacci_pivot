...
async function restoreVwapLevelLock() {
    try {
        const resList = await fetchWithTimeout(`${API_TRADES_URL}${TRADE_API_PATH}?symbol=${SYMBOL_DB}&strategy_name=${STRATEGY_NAME}&level=VWAP&page=0&pageSize=1`);
...
async function hasOpenTradeInDb() {
    try {
        const res = await fetchWithTimeout(`${API_TRADES_URL}${TRADE_API_PATH}?status=OPEN&symbol=${SYMBOL_DB}`);
...
async function getCurrentMetaForOpenTrade(id) {
    const res = await fetchWithTimeout(`${API_TRADES_URL}${TRADE_API_PATH}/${id}`);
    if (res.ok) {
        const json = await res.json();
...
async function reserveOpenTradeInDb({
    ...
    const res = await fetchWithTimeout(`${API_TRADES_URL}${TRADE_API_PATH}`, {
        method: "POST",
...
async function closeReservedTradeAsFailed(id, failureReason, extraMeta = {}) {
    let currentMeta = {};
    try {
        currentMeta = await getCurrentMetaForOpenTrade(id);
    } catch (e) {
        console.error("❌ closeReservedTradeAsFailed read meta error:", e.message);
        return;
    }
...
async function closeLiveTradeInDb({
    id,
    t,
    exitTs,
    exitPrice,
    ...
    const patch = {
        ...
        meta: {
            ...
            strategy_name: STRATEGY_NAME,
            service_name: SERVICE_NAME,
            trade_group_id: t.tradeGroupId ?? currentMeta?.trade_group_id ?? null,
...
async function openTradeReal({ side, bid, ask, vwap, distBps }) {
    ...
    try {
        ...
        if (DRY_RUN) {
            const entryPrice = Number(side === "LONG" ? ask : bid);
            ...
            try {
                data = await finalizeReservedTradeInDb({
                    ...
                    tpPrice,
                    slPrice,
                    entryOrder: { orderId: "dry_entry_" + reservedTrade.id },
                    tpOrder: { algoId: "dry_tp_" + reservedTrade.id },
                    slOrder: { algoId: "dry_sl_" + reservedTrade.id },
                });
            } catch (e) {
                console.error("❌ Dry run finalize failed:", e.message);
                return null;
            }
            ...
            openTrades.set(data.id, {
                ...
                sessionDay,
                entryOrderId: "dry_entry_" + reservedTrade.id,
                tpAlgoId: "dry_tp_" + reservedTrade.id,
                slAlgoId: "dry_sl_" + reservedTrade.id,
            });
...
async function reconcileOpenTrades() {
    ...
    for (const [id, t] of openTrades.entries()) {
        try {
            ...
            if (DRY_RUN) {
                const currentPrice = t.side === "LONG" ? lastBid : lastAsk;
                if (!currentPrice) continue;

                if (t.side === "LONG") {
                    if (currentPrice >= t.tpPrice) {
                        reason = "TP";
                        triggeredOrderId = t.tpAlgoId;
                        real = { avgPrice: t.tpPrice, status: "FILLED" };
                    } else if (currentPrice <= t.slPrice) {
                        reason = "SL";
                        triggeredOrderId = t.slAlgoId;
                        real = { avgPrice: t.slPrice, status: "FILLED" };
                    }
                } else {
                    if (currentPrice <= t.tpPrice) {
                        reason = "TP";
...
async function restoreOpenTrades() {
    let data = [];
    try {
        const res = await fetchWithTimeout(`${API_TRADES_URL}${TRADE_API_PATH}?status=OPEN&symbol=${SYMBOL_DB}&strategy_name=${STRATEGY_NAME}&service_name=${SERVICE_NAME}`);
        if (res.ok) {
            const json = await res.json();
